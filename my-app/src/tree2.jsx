        ...
        if(this.state.modelList.length){
            const listItems = this.state.modelList.map((number) =>
                <ul className="slide-tree" key={number.projectId}>{this.state.toSonCheckBoxStatus}
                    <TreeItemElement model={number} sendCheckStatus={this.state.toSonCheckBoxStatus} />
                </ul>
            );
            return (
                listItems
...
class TreeItemElement extends React.Component{
    ...
    componentWillReceiveProps(nextProps){
        ...
        this.setState({
            checkBoxStatus:nextProps.sendCheckStatus,
            sendCheckStatus:nextProps.sendCheckStatus
            // open:nextProps.open,
    ...
    checkBoxStatusFn(){
        this.setState(nexState => ({
            ...
            toSonCheckBoxStatus:this.state.checkBoxStatus==2?0:2
        }));
    }
    branchToggle(reload){
             ...
             <div className="slideItem">
                 <div className="slideName ellipsis">
                    <span className="slide-tree-editBox">
                        <a onClick={this.checkBoxStatusFn}>{this.state.checkBoxStatus}要传的值：{this.state.toSonCheckBoxStatus}</a>
